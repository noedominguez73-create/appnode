...
from app import db
from app.models import User, Professional, Comment, Credit, ReferralWithdrawal, Specialty
...
def add_specialty():
    ...
    if Specialty.query.filter_by(name=name).first():
        return error_response('Esta especialidad ya existe', 400)
...
@admin_bp.route('/especialidades/<int:id>', methods=['DELETE'])
@admin_required
def delete_specialty(id):
    """Delete a specialty"""
    specialty = Specialty.query.get(id)
    if not specialty:
        return error_response('Especialidad no encontrada', 404)
