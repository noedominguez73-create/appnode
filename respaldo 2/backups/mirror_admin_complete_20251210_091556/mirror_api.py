    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            if api_key:
                ...
                print(f"DEBUG: API Key prefix: {api_key[:4]}...")
                genai.configure(api_key=api_key)
                # Professional Hair Designer System Prompt
                context_prompt = (
                    "Act as an expert hair designer and AI prompt engineer. "
                    "Analyze this image and describe the hairstyle and color in extreme detail for use in an 8K photorealistic image generator. "
                    "Focus on: precision of the cut (e.g., french bob, pixie, layers), texture (silky, voluminous, wavy), and specific color tones (e.g., icy blonde, chestnut with copper lowlights). "
                    "Be concise but professional. Output ONLY the descriptive prompt text."
                )
                
                # Dynamic Model Discovery Strategy
                # The API is rejecting hardcoded names, so we ask the API what is available.
                models_to_try = []
                try:
                    ...
                    if not models_to_try:
                        # Fallback if list_models implies no access or fails weirdly
                        print("DEBUG: No valid models found in list. Defaulting to hardcoded.")
                        models_to_try = ['models/gemini-1.5-flash-latest', 'models/gemini-1.5-pro-latest']
                        
                    print(f"DEBUG: Discovered models to try: {models_to_try}")
                ...
                for model_name in models_to_try:
                    try:
                        print(f"Attempting prompt generation with model: {model_name}")
                        ...
                        e_msg = str(loop_error)
                        print(f"Model {model_name} failed: {e_msg}")
                        errors.append(f"[{model_name}]: {e_msg}")
                        continue
                
                if not generated_prompt:
                    key_status = f"KeyLoaded: {'Yes' if api_key else 'No'}"
                    key_prefix = f"Prefix: {api_key[:4] if api_key else 'N/A'}"
                    generated_prompt = f"Failed to generate. {key_status}, {key_prefix}. Errors: {' ; '.join(errors)}"
