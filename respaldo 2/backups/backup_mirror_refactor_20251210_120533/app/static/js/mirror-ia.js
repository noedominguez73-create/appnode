...
const DEFAULT_STYLES = [
    { name: "Diario Cómodo", icon: "🛋️", vibe: "VIBE_RELAXED" },
    { name: "Oficina Formal", icon: "💼", vibe: "VIBE_PROFESSIONAL" },
    { name: "Evento de Noche", icon: "🍷", vibe: "VIBE_SEDUCTIVE" },
    { name: "Fiesta Elegante", icon: "🥂", vibe: "VIBE_FESTIVE" },
    { name: "Clásico Retro", icon: "🎞️", vibe: "VIBE_VINTAGE" },
    { name: "Look de Gala", icon: "✨", vibe: "VIBE_LUXURY" }
];
...
function renderStyleSlots() {
    ...
    styleSlots.forEach((slot, index) => {
        ...
        btn.innerHTML = `
            ...
            <span class="absolute top-2 right-2 text-xs opacity-0 group-hover:opacity-50">✏️</span>
        `;
...
function toggleMode(mode) {
    ...
    if (mode === 'style') {
        ...
        btn.innerHTML = "<span>🪄</span> Generar Transformación";
    } else {
        styleSec.classList.add('hidden');
        tryonSec.classList.remove('hidden');
        btn.innerHTML = "<span>🛍️</span> Probar Prendas Seleccionadas";
    ...
    if (inventory.length === 0) {
        container.innerHTML = `
            <div class="col-span-full text-center py-8 bg-yellow-50 rounded-2xl border border-yellow-100 text-yellow-700">
                <p class="font-bold">⚠️ Tu closet está vacío</p>
...
function selectItem(category, id) {
    ...
    if (item) {
        ...
        preview.innerHTML = `<span class="text-2xl opacity-20">👕</span>`;
    }
}
...
async function generateTransformation() {
    ...
    btn.disabled = true;
    btn.textContent = "⏳ Procesando con Sastre Virtual...";
    ...
    try {
        ...
        if (currentMode === 'tryon') {
            ...
            if (result.success) {
                ...
                alert("¡Transformación completada! La imagen incluye metadatos del Sastre Virtual.");
            } else {
            ...
            setTimeout(() => {
                ...
                alert(`¡Transformación '${currentSlot.name}' completada!\n\n(Simulación: Se buscaron prendas con etiqueta ${currentSlot.vibe})`);
                loading.classList.add('hidden');
                resultImg.classList.remove('hidden');
            }, 2000);
        }
        ...
        alert("Error de conexión con el Sastre Virtual.");
        loading.classList.add('hidden');
        placeholder.classList.remove('hidden');
    } finally {
        btn.disabled = false;
        btn.innerHTML = "<span>🪄</span> Generar Otra Vez";
    }
}
