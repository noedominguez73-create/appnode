...
const FinanceCore = {
    ...
    payReceivable(id, amount, accountId) {
        ...
        if (receivable && account && payAmount > 0) {
            ...
            if (receivable.paidAmount >= receivable.amount - 0.01) {
    ...
    getFinancialHealth() {
        ...
        const totalInvoiced = this.data.receivables.reduce((sum, r) => sum + r.amount, 0);
        const totalCollected = this.data.receivables.reduce((sum, r) => sum + (r.paidAmount || 0), 0);
    ...
    getProjectedCashFlow(days = 30) {
        ...
        for (let i = 0; i < days; i++) {
            ...
            const dailyIn = receivablesDue.reduce((sum, r) => sum + (r.amount - (r.paidAmount || 0)), 0);
